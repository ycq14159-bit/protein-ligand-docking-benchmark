#!/usr/bin/env python3
"""Processing 4: frozen docking-ready case construction.
...
BENCHMARK = Path(
    os.environ.get(
        "PROCESSING4_BENCHMARK_ROOT",
        Path(__file__).resolve().parents[2],
    )
).expanduser().resolve()
P2_RUN = BENCHMARK / "processing_2_assembly_ready_structure_preparation/runs/20260810_full_01"
P3_RUN = BENCHMARK / "processing_03_direct_contact_qualification/runs/20260811_full_01"
F5_RUN = BENCHMARK / "filter_05_equivalent_redocking_case/step_05_strict_equivalent_grouping_and_representative_selection/runs/step05_full_v1"
F5_RETAINED = F5_RUN / "output/02_filter5_retained_pairs.tsv.gz"
DEFAULT_CONFIG = BENCHMARK / "processing_04_docking_ready_case_construction/config/processing4_v1.json"
...
def prepare_run(args: argparse.Namespace) -> None:
    ...
    retained = pd.read_csv(F5_RETAINED, sep="\t", compression="gzip", dtype=str).fillna("")
    if len(retained) != 158226 or not retained["pair_id"].is_unique:
    ...
    pairs = dataset_table(P3_RUN / "output/provisional_pairs").to_pandas()
    ...
    provenance = {
        "stage": "processing4_v1.0.0", "created_at": utc(), "mode": args.mode,
        "expected_case_count": int(len(merged)), "filter5_retained_count": 158226,
        "inputs": {
            ...
            "processing3_frozen_marker": str(P3_RUN / "_FROZEN.json"),
            "processing2_frozen_marker": str(P2_RUN / "_FROZEN.json"),
        },
    ...
    print(json.dumps({"run_dir": str(run), "cases": len(merged),
                      "buckets": int(merged["bucket_id"].nunique())}, indent=2))
...
def build_frozen_ligand(atom_rows: pd.DataFrame, bond_rows: pd.DataFrame,
    ...
    descriptor = Chem.MolFromSmiles(ccd["descriptor"]) if ccd["descriptor"] else None
    if descriptor is None:
        raise ValueError("CCD canonical descriptor SMILES missing or unparseable")
    descriptor = Chem.RemoveHs(descriptor)
    match = mapped.GetSubstructMatch(descriptor, useChirality=False)
    if not match or len(match) != descriptor.GetNumAtoms():
        raise ValueError("CCD descriptor graph cannot be mapped to frozen observed graph")
...
def independent_start(graph_mol: Chem.Mol, config: dict[str, Any]) -> tuple[Chem.Mol, int]:
    ...
    if code != 0:
        raise RuntimeError(f"ETKDGv3 returned {code}")
    if not AllChem.UFFHasAllMoleculeParams(with_h):
        raise RuntimeError("UFF parameters unavailable")
...
def create_case(tmp: Path, case_row: pd.Series, rec_rows: pd.DataFrame,
    ...
    if lig_atoms.empty or lig_bonds.empty:
        raise ValueError("frozen ligand atoms/bonds missing")
    ...
    mol_ref, ligand_info = build_frozen_ligand(lig_atoms, lig_bonds, ccd_store.component(component))
    ...
    try:
        mol_start, uff_code = independent_start(graph_only, config)
    except Exception as exc:
        raise LigandStartFailure(str(exc)) from exc
    write_sdf(tmp / "ligand_start.sdf", mol_start, cid, "independent_etkdgv3_uff_start")
    ...
    if len(keys) != 1:
        raise RuntimeError(f"ligand graph identity mismatch: {sorted(keys)}")
    ...
    metadata = {
        ...
        "native_pose_leakage_control": "start_function_received_coordinate_free_graph_only; no fallback",
        "created_at": utc(),
    }
...
def run_bucket(run_dir: str, bid: int, force: bool = False,
    ...
    ccd = CCDStore(P2_RUN / "input/ccd_active_snapshot.sqlite")
    ...
    for _, row in inventory.iterrows():
        ...
        if (target / "_SUCCESS.json").exists() and not force:
            results.append({"case_id": cid, "pair_id": row["pair_id"], "status": "P4_DOCKING_READY",
                            "reason": "RESUMED_EXISTING_SUCCESS", "receptor_atoms": None, "ligand_heavy_atoms": None})
            continue
        ...
        try:
            ...
            status = "P4_LIGAND_START_GENERATION_FAILED" if isinstance(exc, LigandStartFailure) else "P4_PREPARATION_REVIEW"
            error = f"{type(exc).__name__}: {exc}"[:4000]
            atomic_json(tmp / "metadata.json", {"case_id": cid, "pair_id": str(row["pair_id"]),
                                                "pdb_id": str(row["pdb_id"]), "processing4_status": status,
                                                "error": error, "created_at": utc()})
            atomic_json(tmp / "_REVIEW.json", {"status": status, "error": error,
                                               "traceback": traceback.format_exc(limit=8), "created_at": utc()})
            result = {"case_id": cid, "pair_id": row["pair_id"], "status": status, "reason": error,
                      "receptor_atoms": None, "ligand_heavy_atoms": None}
...
def validate_command(args: argparse.Namespace) -> None:
    ...
    for row in statuses[statuses["status"] == "P4_DOCKING_READY"].itertuples() if not statuses.empty else []:
        ...
        if absent or not (root / "_SUCCESS.json").is_file():
            ready_file_errors.append({"case_id": row.case_id, "missing": absent})
    report = {
        "status": "PASS" if not missing and not extras and duplicated == 0 and not ready_file_errors else "FAIL",
        ...
        "ready_file_error_count": len(ready_file_errors), "missing_examples": missing[:20],
...
def freeze_command(args: argparse.Namespace) -> None:
    ...
    atomic_parquet(run / "output_manifest.parquet", pd.DataFrame(manifest))
    marker = {"status": "FROZEN", "stage": "processing4_v1.0.0", "frozen_at": utc(),
