...
def select_pilot(external: pd.DataFrame, all_pairs: pd.DataFrame, count: int) -> pd.DataFrame:
    ...
    selected: list[str] = [p for p in ["4agt"] if p in flags]
    reasons: dict[str, set[str]] = defaultdict(set)
    for p in selected: reasons[p].add("TARGETED_BIOJAVA_ALTLOC_SOURCE_RECONCILIATION")
...
def parse_source_atoms(path: Path, wanted: set[str]) -> tuple[dict[str, dict], str]:
    try:
        ...
        tags.append("_atom_site.label_alt_id")
        output = {sid: {"xyz": [], "units": [], "seen": set(), "altloc_values": set(), "all_altloc_nonblank": True} for sid in wanted}
        for row in block.find(tags):
            ...
            altloc = clean(v[15]); output[sid]["all_altloc_nonblank"] = output[sid]["all_altloc_nonblank"] and bool(altloc)
            if altloc: output[sid]["altloc_values"].add(altloc)
        ...
        for value in output.values():
            ...
            value["altloc_values"] = sorted(value["altloc_values"])
        return output, ""
...
def process_bucket(task: tuple) -> dict:
    ...
    instances = []; contacts_out = []; refs = []; errors = list(target_errors); source_mismatch = 0; reconciliations = []
    ...
    for pdb, prows in sorted(by_pdb.items()):
        ...
        for row in prows:
            ...
            if not target: error = "FROZEN_BINDING_RESIDUE_ATOMS_MISSING"
                ...
                actual=len(sources[sid]["xyz"]); expected=int(float(row["source_heavy_atom_count"]))
                # BioJava's atom iterator can expose the sole named altloc container and
                # its atoms twice.  Preserve the unique frozen mmCIF atom identities and
                # accept only this narrowly auditable 2x/all-one-named-altloc signature.
                altloc_ok=(expected==2*actual and actual>0 and sources[sid]["all_altloc_nonblank"] and len(sources[sid]["altloc_values"])==1)
                if altloc_ok:
                    reconciliations.append({"pdb_id":pdb,"source_object_id":sid,"external_instance_id":row["crystal_instance_id"],
                        "raw_unique_heavy_atoms":actual,"step2_biojava_heavy_atom_count":expected,"altloc_values":";".join(sources[sid]["altloc_values"]),
                        "reconciliation_status":"BIOJAVA_SOLE_NAMED_ALTLOC_CONTAINER_2X"})
                else:
                    source_mismatch += 1; error = f"SOURCE_HEAVY_ATOM_COUNT_MISMATCH:{actual}!={expected}"
            if error:
                metric = {"contacts": [], "ext_indices": set(), "target_atoms": set(), "pair_count": 0, "min_distance": math.nan,
                          "direct": False, "bridged": set(), "true_units": set(), "true_ext": set()}
    ...
    write_rows(work/"source_count_reconciliations.tsv", ["pdb_id","source_object_id","external_instance_id","raw_unique_heavy_atoms","step2_biojava_heavy_atom_count","altloc_values","reconciliation_status"], reconciliations)
    return {"bucket": bucket, "input_instances": len(records), "output_instances": len(instances), "contact_rows": len(contacts_out),
            ...
            "errors": len(errors), "source_count_mismatches": source_mismatch, "source_count_reconciliations":len(reconciliations), "runtime_seconds": time.time()-started}
...
def pair_output(step3_pairs: pd.DataFrame, instances: pd.DataFrame, bridged: pd.DataFrame, contacts: pd.DataFrame) -> pd.DataFrame:
    ...
    inst_agg=instances.groupby("pair_id",sort=False).agg(
        n_external_instances_step4=("external_instance_id","size"),
        n_external_instances_contacting_binding_residues=("instance_binding_residue_crystal_contact_4A","sum"),
        min_external_binding_residue_distance_A=("min_external_binding_residue_distance_A","min"),
        step4_error_count=("step4_instance_status",lambda x:int(x.eq("ERROR").sum())),
    ).reset_index().rename(columns={"pair_id":"candidate_pair_id"})
    if len(bridged):
        bind_agg=bridged.groupby("pair_id",sort=False).agg(
            binding_residue_count=("binding_residue_id","size"),
            n_crystal_bridged_binding_residues=("crystal_bridged_binding_residue","sum"),
        ).reset_index().rename(columns={"pair_id":"candidate_pair_id"})
    else:
        bind_agg=pd.DataFrame(columns=["candidate_pair_id","binding_residue_count","n_crystal_bridged_binding_residues"])
    if len(contacts):
        true_frame=contacts[contacts["external_to_binding_residue_contact_4A"].map(truth)]
        unit_agg=(true_frame.drop_duplicates(["pair_id","external_instance_id","contact_unit_id"]).groupby("pair_id",sort=False).size()
                  .rename("n_external_contact_units_contacting_binding_residues").reset_index().rename(columns={"pair_id":"candidate_pair_id"}))
    else:
        unit_agg=pd.DataFrame(columns=["candidate_pair_id","n_external_contact_units_contacting_binding_residues"])
    agg=inst_agg.merge(bind_agg,on="candidate_pair_id",how="left").merge(unit_agg,on="candidate_pair_id",how="left")
    agg["fraction_binding_residues_crystal_bridged"]=agg["n_crystal_bridged_binding_residues"]/agg["binding_residue_count"]
    agg["pair_binding_residue_crystal_contact_4A"]=agg["n_crystal_bridged_binding_residues"].fillna(0).gt(0)
    out = step3_pairs.merge(agg, on="candidate_pair_id", how="left")
...
def observed_controls(instances: pd.DataFrame, contacts: pd.DataFrame, bridged: pd.DataFrame, external: pd.DataFrame, pair_frame: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "case16_only_external_instances_entered": bool(external["equivalence_status"].eq("EXTERNAL_CRYSTAL_INSTANCE").all()),
...
def main() -> None:
    ...
    reconciliation_header=["pdb_id","source_object_id","external_instance_id","raw_unique_heavy_atoms","step2_biojava_heavy_atom_count","altloc_values","reconciliation_status"]
    reconciliations=concat_tsv([w/"source_count_reconciliations.tsv" for w in works],output/"source_count_reconciliations.tsv.gz",reconciliation_header)
    ...
    frozen_binding_keys=set(map(tuple,bridged[["pair_id","binding_residue_id"]].astype(str).to_numpy()))
    contact_binding_keys=set(map(tuple,contacts[["pair_id","binding_residue_id"]].astype(str).to_numpy())) if len(contacts) else set()
    science_rows=pair_frame[pair_frame["step3_status"].eq("SUCCESS_NO_DIRECT_LIGAND_CONTACT")]
    checks={"step4_input_pair_accounting":len(scientific_pair_ids)==len(set(external["candidate_pair_id"])),"instance_accounting":len(instances)==len(external),
        ...
        "source_count_reconciliations_narrow_signature":bool(reconciliations["reconciliation_status"].eq("BIOJAVA_SOLE_NAMED_ALTLOC_CONTAINER_2X").all()) if len(reconciliations) else True,
        ...
        "all_contact_targets_are_frozen_binding_residues":contact_binding_keys.issubset(frozen_binding_keys),
        ...
        "pair_iff_bridged_residue":bool((science_rows["pair_binding_residue_crystal_contact_4A"].map(truth)==pd.to_numeric(science_rows["n_crystal_bridged_binding_residues"]).gt(0)).all()),
        "pair_external_instance_counts_match_step3":bool((pd.to_numeric(science_rows["n_external_instances_step3"]).astype(int)==pd.to_numeric(science_rows["n_external_instances_step4"]).astype(int)).all()),
    ...
    validation={"run_mode":args.mode,"validated_at":utc(),"validation_pass":all(checks.values()),"checks":checks,"boundary_tests":boundaries,
        ...
        "counts":{"pairs":len(pair_frame),"step4_pairs":len(scientific_pair_ids),"external_instances":len(instances),"contact_candidate_rows":len(contacts),
                  ...
                  "contacting_instances":true_instances,"reference_rows":len(refs),"source_count_reconciliations":len(reconciliations),"errors":sum(x["errors"] for x in results)},"pair_status_counts":dict(status_counts)}
    ...
    datasets={"01_external_instance_binding_residue_contact":instances,"02_external_contact_unit_binding_residue_contacts":contacts,
              ...
              "source_count_reconciliations":reconciliations}
