...
def load_context(cfg: dict) -> pd.DataFrame:
    ...
    matrices = frame[BA_R_COLS].to_numpy(float).reshape(-1, 3, 3)
    frame["operator_determinant_error"] = np.abs(np.linalg.det(matrices) - 1.0)
    frame["operator_orthogonality_error"] = np.max(
        np.abs(np.matmul(np.transpose(matrices, (0, 2, 1)), matrices) - np.eye(3)), axis=(1, 2))
    quality_tol = float(cfg["equivalence"]["assembly_operator_quality_tolerance"])
    frame["operator_quality_status"] = np.where(
        (frame["operator_determinant_error"] <= quality_tol) &
        (frame["operator_orthogonality_error"] <= quality_tol), "PASS", "BA_OPERATOR_MATRIX_REVIEW")
    return frame
...
def choose_pilot(pairs: pd.DataFrame, neighbors: pd.DataFrame, sources: pd.DataFrame,
    ...
    for (pdb, _aid), group in context.groupby(["pdb_id", "assembly_id"], sort=False):
        ...
        if group["operator_quality_status"].eq("BA_OPERATOR_MATRIX_REVIEW").any(): flags[pdb].add("ba_operator_matrix_review")
...
def build_ba_inventory(sources: pd.DataFrame, pairs: pd.DataFrame, context: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
        ...
        for source in source_map.get((pdb.lower(), model), []):
            ...
            for ctx in contexts:
                ...
                raw_rows.append({
                    ...
                    "assembly_operator_expression": tsv_safe(ctx["oper_expression_raw"]), "operator_path": tsv_safe(ctx["operator_path"]),
                    "operator_is_composed": bool(ctx["operator_is_composed"]), "operator_is_identity": bool(ctx["operator_is_identity"]),
                    "operator_quality_status": ctx["operator_quality_status"],
                    "operator_determinant_error": float(ctx["operator_determinant_error"]),
                    "operator_orthogonality_error": float(ctx["operator_orthogonality_error"]),
    ...
    for _, group in raw.groupby(key_cols, sort=False, dropna=False):
        ...
        first["operator_quality_status"] = "PASS" if group["operator_quality_status"].eq("PASS").any() else "BA_OPERATOR_MATRIX_REVIEW"
        first["operator_determinant_error"] = float(group["operator_determinant_error"].min())
        first["operator_orthogonality_error"] = float(group["operator_orthogonality_error"].min())
    ...
    ordered = ["ba_instance_id", "pdb_id", "assembly_id", "model_id", "source_object_id", "source_object_type",
               ...
               "operator_is_identity", "operator_quality_status", "operator_determinant_error",
               "operator_orthogonality_error", "provenance_multiplicity"] + \
...
def compare_neighbors(neighbors: pd.DataFrame, inventory: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    for key, group in inventory.groupby(["pdb_id", "assembly_id", "model_id", "source_object_id"], sort=False):
        index[tuple(map(str, key))] = {
            ...
            "quality": group["operator_quality_status"].to_numpy(),
        }
    results = []
        ...
        if result is None:
            ...
            if group is None:
                result = {"same_source_in_ba": False, "best_ba_instance_id": "", "best_ba_operator_path": "",
                          ...
                          "best_ba_operator_quality_status": "",
                ...
                if within and group["quality"][best] != "PASS":
                    status, reason, cv = "BA_EQUIVALENCE_REVIEW", "BEST_BA_OPERATOR_MATRIX_REVIEW", "UPSTREAM_OPERATOR_REVIEW"
                elif within:
                ...
                result = {"same_source_in_ba": True, "best_ba_instance_id": group["ids"][best],
                          ...
                          "best_ba_operator_quality_status": str(group["quality"][best]),
...
def pair_inventory(pairs: pd.DataFrame, eq: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in out.itertuples():
        if str(row.step1_status).startswith("NO_NEIGHBOR"): value = "UPSTREAM_NO_NEIGHBOR"
...
def validate(mode: str, pairs_in: pd.DataFrame, neighbors_in: pd.DataFrame, inventory: pd.DataFrame,
    ...
    checks = {
        ...
        "step1_fields_inherited_unchanged": eq[neighbors_in.columns].reset_index(drop=True).equals(neighbors_in.reset_index(drop=True)),
        "upstream_no_neighbor_status_mapped": bool(
            pair_out.loc[pair_out["step1_status"].astype(str).str.startswith("NO_NEIGHBOR"), "step2_status"]
            .eq("UPSTREAM_NO_NEIGHBOR").all()),
        ...
        "tsv_control_characters_zero": not inventory[["assembly_gen_row_id", "assembly_operator_expression", "operator_path"]]
            .astype(str).apply(lambda s: s.str.contains(r"[\t\r\n]", regex=True).any()).any(),
    }
    if mode == "full":
        checks["full_expected_instance_count"] = len(eq) == expected_neighbors
        checks["full_expected_pair_count"] = len(pair_out) == expected_pairs
        checks["full_expected_upstream_no_neighbor_count"] = int(pair_out["step2_status"].eq("UPSTREAM_NO_NEIGHBOR").sum()) == int(cfg["validation"]["expected_full_no_neighbor_pairs"])
...
def finalize(run: Path, frames: dict[str, pd.DataFrame], validation: dict, cfg: dict,
    ...
    for name, frame in paths.items():
        with gzip.open(output / name, "rt", encoding="utf-8", newline="") as fh:
            physical_rows = max(0, sum(1 for _ in fh) - 1)
        serialized_counts[name] = {"expected_rows": len(frame), "physical_rows": physical_rows,
                                   "match": physical_rows == len(frame)}
    validation["checks"]["serialized_physical_row_counts_match"] = all(x["match"] for x in serialized_counts.values())
    validation["serialization_row_counts"] = serialized_counts
    validation["validation_pass"] = all(validation["checks"].values())
    ...
    if validation["validation_pass"]:
        ...
        raise RuntimeError("post-serialization validation failed")
