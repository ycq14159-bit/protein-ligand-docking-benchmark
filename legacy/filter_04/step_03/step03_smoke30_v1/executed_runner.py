...
def observed_controls(instances: pd.DataFrame, units: pd.DataFrame, external_input: pd.DataFrame, pairs: pd.DataFrame) -> dict:
    ...
    return {"case1_one_atom_unit_false": bool((one & ~units["contact_unit_direct_crystal_contact_4A"]).any()) if len(units) else False,
            ...
            "case13_only_external_instances_entered": len(external_input)==355846 or external_input["equivalence_status"].eq("EXTERNAL_CRYSTAL_INSTANCE").all(),
...
def main() -> None:
    ...
    if args.mode == "pilot": checks["all_required_scientific_controls_present"] = all(controls.values())
    if args.mode == "full":
