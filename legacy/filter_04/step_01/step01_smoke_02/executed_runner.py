...
def read_bucket(root: Path, bucket: int, columns: list[str]) -> pd.DataFrame:
    path = bucket_path(root, bucket)
    if not path.exists():
        return pd.DataFrame(columns=columns)
    return ds.dataset(path, format="parquet").to_table(columns=columns).to_pandas(split_blocks=True, self_destruct=True)
...
def prepare_bucket(task: tuple[int, list[dict], dict, str, bool, list[str]]) -> dict:
    ...
    pocket = read_bucket(p3 / "pair_pocket_residues", bucket, ["pair_id", "chain_instance_id", "protein_residue_id"])
